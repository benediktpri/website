...
const CATEGORY_CONFIG: { key: string; label: string; color: string; types: Set<string> }[] = [
  { key: "running", label: "Run", color: "#70CF25", types: RUN_TYPES },
  { key: "cycling", label: "Ride", color: "#FC5200", types: CYCLING_TYPES },
  { key: "strength", label: "Workout", color: "#997700", types: STRENGTH_TYPES },
];
...
function aggregateYears(activities: StravaActivity[]): YearSummary[] {
  ...
  for (const [year, yearActivities] of byYear) {
    ...
    const daysSoFar = year < now.getFullYear()
      ? daysInYear
      : Math.floor((now.getTime() - new Date(year, 0, 1).getTime()) / (24 * 60 * 60 * 1000)) + 1;
    ...
    const catMap = new Map<string, { count: number; days: Set<string>; distance: number; time: number }>();
    for (const cat of [...CATEGORY_CONFIG, { key: "other", label: "Other", color: "#44403c", types: new Set<string>() }]) {
    ...
    const allConfigs = [
      ...CATEGORY_CONFIG,
      { key: "other", label: "Other", color: "#44403c", types: new Set<string>() },
    ];
...
async function main() {
  if (!CF_ACCOUNT_ID || !CF_KV_NAMESPACE_ID || !CF_API_TOKEN) {
    if (existsSync(OUTPUT_PATH)) {
      console.warn(
        "Missing CF env vars — keeping existing strava.json",
      );
    } else {
      console.warn(
